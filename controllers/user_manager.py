...
class UserManager:
            ...
            if not os.path.exists("users.json"):
                ...
                with open("users.json", "r") as file:
                    ...
                    if content:
                        ...
                        debug_log("'users.json' is empty or contains only whitespaces, returning [].")
                        return []
    ...
    def save_users(self, users):
        ...
        with open("users.json", "w") as file:
            ...
            debug_log("'users.json' overwritten! ")
        
    # Adds a user to the list.
    def add_user(self, users_file, new_user):
        debug_log("Start of function add_user... ")
        debug_log(f"users_file --> {users_file}")
        debug_log(f"new_user --> {new_user.__dict__}")
    ...
    def save_user(self, user):
        ...
        debug_log(f"user data:\n{user_data}")
        with open(filename, 'w') as file:
            json.dump(user_data, file, indent=4)
            debug_log("user saved successfully")
    ...
    def load_user(cls, user_id):
        ...
        for _, user in enumerate(users): # Iterates users.json in search of an user with the ID provided.
            if user.get("id") == str(user_id):
                debug_log(user)
                debug_log(f"user {user.get('username')} found with id {user_id}")
                debug_log(f"returning user...")
